...
const AddUser = () => {
  ...
  for (let i = 0; i < statusList.length; i++) {
    childrens.push(<Option key={statusList[i].value}>{statusList[i].name}</Option>);
  }
  useEffect(() => {
   (async()=>{
  ...
  const onFinish = async(values: any)=>{
    // Check use exist 
    let index = users.findIndex(item=>item.tenDangNhap.toLocaleLowerCase() === values.tenDangNhap.toLocaleLowerCase())
    if(index !== -1){
      Swal.fire({
        title: 'Error!',
